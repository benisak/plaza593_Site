...
const SellCard: React.FC<SellCardProps> = ({ price = "999" }) => {
  const renderStars = () => {
  ...
  // Calculate installment price (dividing by 3 for 3 installments)
  const calculateInstallment = (priceValue: string) => {
    const numericPrice = parseInt(priceValue.replace(/[^0-9]/g, ""));
    const installment = Math.round(numericPrice / 3);
