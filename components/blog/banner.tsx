"use client";
import React, { useState, useEffect, useRef, JSX, useMemo } from "react"; //aquiiiii
...
export const BannerAd: React.FC<BannerAdProps> = ({ ingredients = [] as Ingredient[] }) => {
    ...
    <>
      ...
      >
        ...
        <div className="relative flex items-center justify-center pt-6">
          <h2 className="font-inter text-[15px] font-bold text-black">🛒 Ingredients</h2>
          <a
            id="close-menu"
            {...(memorizedIngredientLinks[0]?.linkProps || {})} //aquiiiii
            target="_blank"
            rel="noopener noreferrer"
        ...
        <ul className="flex mt-0 p-4 space-x-3 overflow-x-auto">
          {memorizedIngredientLinks.map((ingredient, index) => ( //aquiiiii
            ...
            {/* Left-aligned Image (same as text) */}
              ...
              <div className="text-left">
                <a {...ingredient.linkProps} target="_blank" rel="noopener noreferrer" className="transition hover:text-green-600"> 
                  <h3 className="text-sm font-normal leading-tight text-[#007185] line-clamp-2">{ingredient.title}</h3>
                </a>
