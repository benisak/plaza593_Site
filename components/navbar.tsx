...
function NavbarContent({
  ...
  const MAX_CATEGORIES = 14; // Define the maximum number of categories to display in te dropdown of mTodas las categorías 
      ...
      <div className="relative block w-full bg-white px-4 py-4 md:hidden">
        <div className="flex items-center justify-between">
          {/* Logo */}
           <Link
            href="/"
            className="block w-[117px] h-[36px]"
          >
            <Image
              src="/img/logo593_mobile.svg"
              alt="De todo Market Logo"
              width={135}
              height={36}
              priority
                      ...
                      {rightmenu.map((category, index) => (
                        <li key={`${category.label}-${index}`}>
                          <Link
                            href={category.href}
                            onClick={() => {
                              toggleOverlayMenu(); // Close "Todas las categorías " overlay
